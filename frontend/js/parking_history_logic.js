...
let getPrkingHistory = async () =>{
    console.log("getParkingHistory");
    const tableBody = table.querySelector('tbody');

    try {
        const resp = await fetch(`${serverURL}parking-history/all-details`);
        ...
        for(const rows of data){
            ...
            for(const colText of rowArray){
                ...
                // rupeeSymbol.innerHTML = '<i class="fa fa-inr"></i> ';
                const colElem = document.createElement('td');
                colElem.textContent = colText;
