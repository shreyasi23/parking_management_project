...
getVehicleNo.addEventListener('submit', async (e) => {
    ...
    if (vehicleNo && exit_time && payment_method) {
        try {
            const resp4 = await fetch(`${serverURL}parking-area-info`);
        ...
        try {
            const resp1 = await fetch(`${serverURL}parking-history/${vehicleNo}`);
        ...
        try {
            const resp5 = await fetch(`${serverURL}parking-history/exit-time`, {
                method: 'PUT',
                headers: {
    ...
    // get parking slot of that vehicle -- do not uncomment
    // try {
    //     const resp1 = await fetch(`${serverURL}parking-history/${vehicleNo}`);
    //     const parkingInfo = await resp1.json();
    //     console.log(parkingInfo);
    ...
    try {
        const resp3 = await fetch(`${serverURL}parking-slots/${slot}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
...
const calculateBill = async () => {
    ...
    billElem.innerHTML = `<div>
        ...
        <div id="box">
            ...
            <div>
                <p><span><i class="fa fa-inr"></i></span>${base_price}</p>
                <p><span><i class="fa fa-inr"></i></span>${per_hr_charge * extra}</p>
                <p>${entry_time}</p>
                <p>${exit_time}</p>
                <h4><strong><span><i class="fa fa-inr" id="ttl"></i></span>${bill_amt}</strong></h4>
            </div>
        </div>
    </div>
    <div class="row">
    ...
    try {
        const resp2 = await fetch(`${serverURL}/invoice/create-invoice`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
