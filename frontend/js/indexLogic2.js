...
document.getElementById('flex_cont').firstElementChild.textContent = currentDate;
...
let getDBData = async () => {
    ...
    try {
        const resp = await fetch(`${serverURL}parking-history/occupied-5`);
    ...
    try{
        const resp4 = await fetch(`${serverURL}invoice/${currentDate}`);
        ...
        total_amt.textContent = amount2+".00";
        if(!resp4.ok){
...
// refresh for refreshing data displayedon the page
// setInterval(, 5000);